...
router.post('/talk', (req, res) => {
    ...
    if (!title){
        const incomplete_data = new Response(httpStatus.BAD_REQUEST, 'Kindly provide all title for the talk', res, true)
    ...
    return talksModel.create(data)
        .then(() => {
            const savedTalk = new Response(httpStatus.CREATED, 'Talk has been registered successfully', res, false);
            return savedTalk.res_message();
        })
        .catch((err) => {
            console.log('Error from saving talk', err);
            const unsavedTalk = new Response(httpStatus.NOT_FOUND, 'Unable to register Talk', res, true);
...
router.post('/attendee', (req, res) => {
    ...
    return attendeesModel.create(data)
        .then(() => {
            const savedAttendee = new Response(httpStatus.CREATED, 'Attendee has been registered successfully', res, false);
            return savedAttendee.res_message();
        })
        .catch((err) => {
            console.log('Error from saving Attendee', err);
            const unsavedAttendee = new Response(httpStatus.NOT_FOUND, 'Unable to register attendee', res, true);
    ...
    return attendeesModel.findByName(name)
        .then((data) => {
            ...
            return talksModel.findOneAndUpdate(query, { $push: { attendees : att }} )
                .then(() => {
                    const updatedTalk = new Response(httpStatus.CREATED, 'Attendee has been added to talk successfully', res, false);
                    return updatedTalk.res_message();
                })
                .catch((err) => {
                    console.log('update error', err)
                    const unUpdatedTalk = new Response(httpStatus.NOT_FOUND, 'Unable to add attendee to talk', res, true);
                    return unUpdatedTalk.res_message();
                })
        })
        .catch((err) => {
            console.log('Error finding by name', err);
            const unFoundName = new Response(httpStatus.BAD_REQUEST, 'Unable to find attendee name', res, true);
...
router.delete('/talk/:title', (req, res) => {
    ...
    talksModel.deleteOne({ title: title })
        .then(() => {
            const removeTalk = new Response(httpStatus.OK, 'Talk has been removed successfully', res, false);
            return removeTalk.res_message();
        })
        .catch((err) => {
            console.log('err from deleting', err);
            const unremovedTalk = new Response(httpStatus.NOT_FOUND, 'Unable to remove talk', res, true);
